...
# run app
java -jar target/herro-java-03-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/herro-java-03:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/herro-java-03 -n dev

# get external endpoint
kubectl get svc herro-java-03-svc -n dev
```
...
```bash
kubectl port-forward svc/herro-java-03-svc 8080:80 -n dev
...
kubectl rollout restart deploy/herro-java-03 -n dev
kubectl rollout status deploy/herro-java-03 -n dev
```
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/herrojava03/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© christian.herro@morningstar.com. For demo/PoV purposes.
